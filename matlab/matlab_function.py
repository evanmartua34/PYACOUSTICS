...
def detect_syllable_nuclei(opt,path_to_files, output_path):
	files=[]
...
def fu_sylncl_sub(s,opt):
	...
	t_nou = np.unique(np.transpose(np.array([t_nou find(voi==0)])))
	...
	lmpopt.peak.mpd = math.floor(opt.fs * opt.md / sts)
...
def fu_sylbnd(s, sn ,opt):
	...
	for i in range (1,len(sn)):
		...
		sb = np.array([sb, on+all_i[ymini(1)]]);
...
def fu_locmax(y,opt):
	...
	idx = scipy.signal.find_peaks(fu_smooth(y,opt.smooth),'MINPEAKDISTANCE',opt.peak.mpd,'MINPEAKHEIGHT',opt.peak.mph, 'THRESHOLD',opt.peak.th)
	pks = ys[idx]
